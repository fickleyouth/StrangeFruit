...
class View extends BuildHTML {
	...
	$parent_li_attributes = array("id"=>$parent_id);
	$li_start_parent = BuildHTML::start_element(self::$li, $parent_li_attributes);
	$parent_link_attributes = array('href'=>'?node='.$parent_link,'title'=>$parent_desc);
	$parent_link_start = BuildHTML::start_element(self::$link, $parent_link_attributes);
	$parent_link_end = BuildHTML::end_element(self::$link);
	//this was abstracted using BuildHTML start element
	echo $li_start_parent.$parent_link_start.$parent_name.$parent_link_end;
	if (is_array($val)) {
	echo $ul_start;
	...
	$child_li_attributes = array("id"=>$child_id);
	$li_start_child = BuildHTML::start_element(self::$li, $child_li_attributes);
	$child_link_attributes = array('href'=>'?node='.$child_link,'title'=>$child_desc);
	$child_link_start = BuildHTML::start_element(self::$link, $child_link_attributes);
	$child_link_end = BuildHTML::end_element(self::$link);
	//this was abstracted using BuildHTML start element
	echo $li_start_child.$child_link_start.$child_name.$child_link_end.$li_end;
	}
	}
	echo $ul_end.$li_end;
	}
	...
	echo $related_start;
	echo $ul_start;
	echo $li_start.'Related Material'.$li_end;
	...
	function draw_header($main_menu) {
		...
		$logo_attributes = array("src"=>DESIGN_IMAGES_DIR.$logo, "alt"=>"framework default logo", "title"=>"home", "class"=>"logo");
		$header_start = BuildHTML::start_element(self::$div, $attributes);
		$header_end = BuildHTML::end_element(self::$div);
		$logo_container_attributes = array("id"=>"header_logo", "class"=>"grid_3");
		$logo_container_start = BuildHTML::start_element(self::$div, $logo_container_attributes);
		$logo_container_end = BuildHTML::end_element(self::$div); 
		$logo_link_attributes = array("href"=>"http://localhost/StrangeFruit/?node=content/text&id=1");
		$logo_link_start = BuildHTML::start_element(self::$link, $logo_link_attributes);
		$logo_link_end = BuildHTML::end_element(self::$link);
		$logo_start = BuildHTML::start_element(self::$img, $logo_attributes);
		echo $header_start;
		//This was abstracted further and amended to include a link to the home page
		echo $logo_container_start.$logo_link_start.$logo_start.$logo_link_end.$logo_container_end;
